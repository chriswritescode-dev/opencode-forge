...
function makeClient(messagesFn: ReturnType<typeof mock>): OpencodeClient {
  return {
    session: {
...
describe('fetchLatestPlanForSession', () => {
  test('returns the marked plan text when present in the latest assistant message', async () => {
    const messages = mock(async () => ({ data: [assistantMessage(`Some preamble\n${VALID_PLAN}\nSome closing words`)] }))
  ...
  test('omits `directory` from the SDK call when not provided', async () => {
    const messages = mock(async () => ({ data: [assistantMessage(VALID_PLAN)] }))
    const client = makeClient(messages)
  ...
  test('returns null and logs when the messages call errors', async () => {
    const messages = mock(async () => ({ error: new Error('boom') }))
    const client = makeClient(messages)
    const debug = mock(() => {})
  ...
  test('debug callback receives a string for the success path', async () => {
    const messages = mock(async () => ({ data: [assistantMessage(VALID_PLAN, 'msg-123')] }))
    const client = makeClient(messages)
    const debug = mock(() => {})
