...
import { buildLoopPermissionRuleset, buildAuditSessionPermissionRuleset, evaluatePermissionRuleset } from '../src/constants/loop'
...
describe('evaluatePermissionRuleset', () => {
  test('allow-all rule matches everything not later denied', () => {
    expect(evaluatePermissionRuleset(buildLoopPermissionRuleset(), { permission: 'read' })).toBe('allow')
  })

  test('later deny overrides earlier allow-all', () => {
    expect(evaluatePermissionRuleset(buildLoopPermissionRuleset(), { permission: 'review-write' })).toBe('deny')
  })

  test('git push deny matches by glob pattern', () => {
    expect(evaluatePermissionRuleset(buildLoopPermissionRuleset(), { permission: 'bash', pattern: 'git push origin main' })).toBe('deny')
  })

  test('git pull is allowed (only push is denied)', () => {
    expect(evaluatePermissionRuleset(buildLoopPermissionRuleset(), { permission: 'bash', pattern: 'git pull' })).toBe('allow')
  })

  test('external_directory denied', () => {
    expect(evaluatePermissionRuleset(buildLoopPermissionRuleset(), { permission: 'external_directory', pattern: '/tmp/x' })).toBe('deny')
  })

  test('no matching rules returns ask', () => {
    expect(evaluatePermissionRuleset([], { permission: 'bash' })).toBe('ask')
  })

  test('audit ruleset: edit is denied', () => {
    expect(evaluatePermissionRuleset(buildAuditSessionPermissionRuleset(), { permission: 'edit' })).toBe('deny')
  })

  test('audit ruleset: read is allowed', () => {
    expect(evaluatePermissionRuleset(buildAuditSessionPermissionRuleset(), { permission: 'read' })).toBe('allow')
  })
})
