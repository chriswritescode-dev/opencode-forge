...
describe('SandboxManager host access', () => {
  ...
  describe('envFileMounts', () => {
    ...
    test('mounts .env when present in sourceProjectDir', async () => {
      ...
      const opts = getCreateContainerOpts(mockDocker)
      const extraMounts = (opts?.extraMounts as string[]) ?? []
      expect(extraMounts).toContain(`${tmpDir}/.env:/workspace/.env:ro`)
    })

    test('does not mount .env when absent from sourceProjectDir', async () => {
      // tmpDir has no .env file
      const mockDocker = createMockDockerService()
      const logger = createMockLogger()
      const config: SandboxManagerConfig = {
        image: 'oc-forge-sandbox:latest',
      ...
      const opts = getCreateContainerOpts(mockDocker)
      const extraMounts = (opts?.extraMounts as string[]) ?? []
      expect(extraMounts.every(m => !m.includes('.env'))).toBe(true)
    })

    test('mounts custom envFiles paths', async () => {
      writeFileSync(join(tmpDir, '.env.prod'), 'PROD=true\n', 'utf-8')
      ...
      const opts = getCreateContainerOpts(mockDocker)
      const extraMounts = (opts?.extraMounts as string[]) ?? []
      expect(extraMounts).toContain(`${tmpDir}/.env.prod:/workspace/.env.prod:ro`)
    })

    test('does not mount envFiles when sourceProjectDir is not set', async () => {
      const mockDocker = createMockDockerService()
      const logger = createMockLogger()
      const config: SandboxManagerConfig = {
        image: 'oc-forge-sandbox:latest',
        sourceProjectDir: undefined,
      }
      ...
      const opts = getCreateContainerOpts(mockDocker)
      const extraMounts = (opts?.extraMounts as string[]) ?? []
      expect(extraMounts.every(m => !m.match(/\.env/))).toBe(true)
    })
  })
