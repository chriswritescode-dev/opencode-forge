...
- **Execution** — approved-plan launch paths in worktree (`mode: loop`) or project-directory (`mode: 'new-session'`) loops, plus direct `/execute-goal` loops in dedicated worktree sessions; plan loops can also target a configured remote opencode server (see [Configuration](docs/configuration.md#remotes))
...
- **Goal loops** (`execute-goal`, `execute-plan` `mode: 'new-session'`): a completed auditor pass must leave **zero outstanding review findings of any severity** — both bugs and warnings block completion. See [Goal Loops](docs/loop-system.md#goal-loops).
