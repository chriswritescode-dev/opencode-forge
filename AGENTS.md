...
- The loop watchdog has exactly two progress signals, and `busy` is not one of them: `recordActivity` (tool hooks and phase dispatch, keyed by loop) and `recordSessionContent` (streamed `message.part.updated` deltas, keyed by session). The content signal is what keeps a purely-thinking model — which emits no tool calls — from being nudged as wedged, so it is fed from the first branch of `loop.tick` and must stay O(1): no loop resolution, no logging, no awaits. The busy ceiling (`busyStallTimeoutMs`) is measured from the newest of the two across the loop session and its subagent sessions.
...
- `getSandboxState` is the only sandbox liveness primitive; there is deliberately no boolean `isRunning`, because collapsing the states loses the two that matter. `sbx` suspends idle microVMs to `stopped` and `sbx exec` resumes them in place, so `stopped` is reusable and must never trigger a create or an eviction; `unknown` means the status query itself failed and is never evidence of absence. Only a confirmed `missing` may create (`start`, `resolveUsableSandbox`) or evict a map entry (`isLive`). `registerActiveSandbox` is the only place a usable sandbox is recorded in the active map.
