...
- Multiple plugin instances share one process. Ownership gate: `ownsLoopWorktree` in `src/loop/runtime.ts` compares `directory` to `worktreeDir` via `canonicalizePath`. `startWatchdog` and `session.error` handler gate on it — fail-safe, never a hang. Do not extend the gate to the idle handler. The ungated idle handler is safe because each instance only receives its own location's events: V1 delivers per directory, and the V2 bus scopes every plugin subscription to its location, so `src/host/v2.ts` must not add its own location filter.
