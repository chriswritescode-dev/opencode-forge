...
- Persisted review findings are the only channel from the auditor to the coding agent. Coder prompts inline each finding in full; auditor response prose is never injected into a coder prompt, and the only part of it the runtime parses is the section-summary block. `formatFindingDetails` in `src/utils/review-format.ts` is the single finding renderer for both `review-read` and every coder prompt. `lastAuditResult` is persisted for the dashboard and `loop-status` display only.
