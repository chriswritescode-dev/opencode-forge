...
export interface PermissionAskDeps {
  resolver: {
    resolveActiveLoopForSession(sessionId: string): Promise<{ loopName: string; active: boolean; sandbox?: boolean } | null>
  }
  logger: Logger
}
...
export function createPermissionAskHandler(deps: PermissionAskDeps) {
  return async (input: Permission, output: { status?: 'allow' | 'deny' | 'ask' }): Promise<void> => {
    ...
    if (!state.sandbox) {
      deps.logger.log(`[permission.ask] loop=${state.loopName} is not a sandbox loop — falling through to host default`)
      return
    }
