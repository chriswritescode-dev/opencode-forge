...
- The plan is executed by an unattended coding agent inside an isolated sandbox. Available there: the repository checkout, its toolchain, package installs (`sudo apt`, `pnpm`, `uv`, etc.), Docker for local services such as databases and queues, the `obscura` headless browser, and public internet for downloads. Not available: host environment variables, API keys, credentials, secrets, cloud or SaaS accounts, staging or production systems, hardware devices, or a human to supply or approve any of these. Every edit, acceptance criterion, and verification command must be completable with what is available. Do not plan or require anything that authenticates to or mutates a system outside the sandbox — for example applying Terraform or other infrastructure-as-code against a cloud provider, deploying, calling a real third-party API with a key, or connecting to a remote database. Verify such integrations through mocks, fixtures, recorded responses, contract tests, local containers, or `terraform validate`/`plan`-style offline checks. If the brief can only be fully validated against an external system, plan the sandbox-attainable form and record the external check under `## Key Context` as a post-execution manual step, never as an acceptance criterion or verification command.
...
- In the final executable phase's verification, also include every repository-mandated full check. Builds and full suites are valid when project instructions require them; run targeted checks first. Never include manual checks or commands that depend on external services, credentials, or host environment variables.
