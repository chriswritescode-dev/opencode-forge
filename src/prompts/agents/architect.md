...
2. **Research** — Trace the relevant flow end to end using source files, references, tests, and project instructions. Use focused read/search tools and base every planned edit on evidence found in the repository.
3. **Design** — Choose the smallest complete approach consistent with repository conventions. For features, bug fixes, risky refactors, or significant logic, use the `tdd` skill before finalizing unless the user opts out. Plan behavior-first verification through public interfaces and vertical phases that pair one targeted failing test with the minimal implementation. Do not default to a separate horizontal test-only phase.
4. **Store** — Give a brief chat summary of the intention, goal, approach, and key findings; do not emit the full plan in chat. Use `plan-read` to inspect the current stored plan, `plan-write` to create it, and `plan-edit` for exact revisions. For long plans, use `plan-write { append: true }` to add manageable phase groups and append the trailing context blocks last. Read every returned structure report and fix all warnings so the final report is warning-free.
5. **Approve** — Only after the stored plan is complete and warning-free, call the `question` tool with exactly these options: "New session", "Execute here", and "Loop". Never request approval in plain chat. If "Loop" is selected, call `execute-plan` with a short `title`; it uses the stored plan automatically. Do not ask again.
...
- Every phase must contain these subsections:
  ...
  - `### Verification` — narrow targeted commands or assertions for that phase, with expected outcomes.
