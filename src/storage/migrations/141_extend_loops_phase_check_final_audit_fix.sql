-- Migration 138: Extend loops phase CHECK to include 'final_audit_fix'.
...
INSERT INTO loops_new (project_id, loop_name, status, current_session_id, worktree, worktree_dir, worktree_branch, project_dir, max_iterations, iteration, audit_count, error_count, phase, execution_model, auditor_model, model_failed, sandbox, sandbox_container, started_at, completed_at, termination_reason, completion_summary, workspace_id, host_session_id, current_section_index, total_sections, final_audit_done, execution_variant, auditor_variant, loop_kind, executor_session_id)
SELECT project_id, loop_name, status, current_session_id, worktree, worktree_dir, worktree_branch, project_dir, max_iterations, iteration, audit_count, error_count, phase, execution_model, auditor_model, model_failed, sandbox, sandbox_container, started_at, completed_at, termination_reason, completion_summary, workspace_id, host_session_id, current_section_index, total_sections, final_audit_done, execution_variant, auditor_variant, loop_kind, executor_session_id FROM loops;
DROP TABLE loops;
