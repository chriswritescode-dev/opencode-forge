...
2. **No new JSON blob columns**: New tables should have explicit columns for queryable fields. JSON is only used for opaque payloads that are never filtered (e.g., `stats_json` in `graph_status`, `data` in `tui_preferences`).
...
- New typed tables: `100`–`104` (Phase 1 of storage-repos overhaul)
...
- `104_create_graph_status.sql`: Graph indexing status (upsert semantics)
