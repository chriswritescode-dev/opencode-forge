...
// One section: a clickable head (flat status text, no pill) plus an expandable
// body. Status drives the row's left-border accent color.
function SectionItem(props: { sec: DashboardSection; isOpen: () => boolean; onToggle: () => void }) {
  const sec = props.sec
  const duration = createMemo(() => formatSectionDuration(sec.startedAt, sec.completedAt))

  return html`<div class=${() => 'section-item section-item-' + sec.status}>
    <div class="section-head" onclick=${props.onToggle}>
      <span class="section-caret">${() => (props.isOpen() ? '▾' : '▸')}</span>
    ...
    ${() => (props.isOpen() ? SectionBody({ sec }) : '')}
  </div>`
}

// Owns the per-row open/closed state, keyed by sectionIndex so it survives data
// polls (the row map only re-runs when the section count changes).
export function SectionList(props: { sections: () => DashboardSection[] }) {
  ...
  return html`<div class="sections-group">
    <h4>Sections</h4>
    ${() =>
      props.sections().map((sec: DashboardSection) =>
        SectionItem({
          sec,
          isOpen: () => open().has(sec.sectionIndex),
          onToggle: () => toggle(sec.sectionIndex),
        }),
      )}
  </div>`
}
...
// Reuses the .ldh-stat cell styling for the summary row.
...
// CSS-only usage graphs: a summary stat grid, a stacked token-composition bar
// with legend, and per-model cost bars. The full precise numbers remain
// available as hover tooltips via the existing format helpers. Usage has no
// scroll/resize state to preserve, so re-mapping on poll updates is fine.
export function LoopUsage(props: { usage: () => NonNullable<DashboardLoop['usage']> }) {
  const u = () => props.usage()
  const segments = createMemo(() => tokenBreakdownSegments(u()))
  const totalTokens = createMemo(() => segments().reduce((sum, seg) => sum + Math.max(0, seg.value), 0))
  ...
  return html`<div class="usage-group">
    ...
    <div class="ldh-stats" title=${() => formatUsageTotal(u())}>
      ${UsageStat({ label: 'Total Cost', value: () => formatUsageCost(u().totalCost) })}
      ${UsageStat({ label: 'Total Tokens', value: () => formatTokenCount(totalTokens()) })}
      ${UsageStat({ label: 'In / Out', value: () => formatTokenCount(u().totalInputTokens) + ' / ' + formatTokenCount(u().totalOutputTokens) })}
      ${UsageStat({ label: 'Messages', value: () => String(u().totalMessageCount) })}
    </div>
...
export function LoopDetail(props: { dashLoop: DashboardLoop; onBack: () => void }) {
  ...
  return html`<div class="loop">
    ...
    <div class="loop-detail">
      ...
      <!-- Sections -->
      ${() => (hasSections() ? SectionList({ sections: () => dl().sections! }) : '')}
      ...
      <!-- Usage -->
      ${() => (hasUsage() ? LoopUsage({ usage: () => dl().usage! }) : '')}
