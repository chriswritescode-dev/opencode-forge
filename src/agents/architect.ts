...
- **Skills**: Optional. If the plan benefits from specific agent skills during execution (for example skills you loaded while planning, such as \`tdd\`), declare them on a machine-readable line \`Skills: tdd, diagnose\` near the top of the marked plan, immediately after the Loop Name line. The loop loads these skills into the executing code agent and copies them into the worktree so they work in the sandbox. Use comma-separated skill names — not a markdown heading or bullet. Omit the line entirely when no skills apply.
