...
export function terminationReasonToString(reason: TerminationReason): string {
  switch (reason.kind) {
    ...
    case 'decomposer_error':
      return `decomposer_error: ${reason.message}`
...
export function parseTerminationReasonString(str: string): TerminationReason {
  ...
  if (idx >= 0) {
    ...
    if (prefix === 'decomposer_error') return { kind: 'decomposer_error', message }
  ...
  const knownKinds = [
    ...
    'session_creation_failed', 'decomposition_failed',
    'decomposer_prompt_failed', 'audit_retry_exhausted',
